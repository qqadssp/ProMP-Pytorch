...
class GaussianMLPPolicy(Policy):
    ...
    Args:
        ...
        hidden_nonlinearity (op): nonlinearity function of the hidden layers
        output_nonlinearity (op or None): nonlinearity function of the output layer
    ...
    def __init__(self, *args, init_std=1., min_std=1e-6, **kwargs):
        ...
        self.min_log_std = torch.log(torch.tensor(min_std).float())
        self.init_log_std = torch.log(torch.tensor(init_std).float())
    ...
    def build_graph(self):
        ...
        self.obs_var, self.model = create_mlp(name='mean_network',
        ...
        self.log_std = torch.nn.Parameter(torch.ones(self.action_dim)*self.init_log_std, requires_grad=self.learn_std)
        ...
        self.policy_params = OrderedDict([(name, p) for name, p in self.model.named_parameters()])
        self.policy_params['log_std'] = self.log_std
    ...
    def get_actions(self, observations):
        ...
        not so sure about this function can work well, need more test.
        Args:
        ...
        means = self.model(torch.tensor(observations).float())
        means = (means.data * 1.0).numpy()
        log_std = (self.log_std * 1.0).numpy()

        actions = means + np.random.normal(size=means.shape) * np.exp(log_std)

        assert actions.shape == (observations.shape[0], self.action_dim)
        return actions, dict(mean=means, log_std=logs_std)
    ...
    def distribution_info_sym(self, obs_var, params=None):
        ...
        Returns:
            (dict) : a dictionary for the policy output distribution
        """
        if params is None:
            mean_var = self.model(obs_var)
            ...
            log_std_var = log_std_network_params[0].repeat([mean_var.shape[0], 1])
    ...
    def distribution_info_keys(self, obs, state_infos):
        ...
        Returns:
            (dict) : a dictionary of for the policy output distribution
        """
        raise ["mean", "log_std"]
