...
class Policy(Serializable):
    ...
    Note:
        the preupdate policy is stored as Variables, while the postupdate
        policy is stored in list of torch.tensor(graph_fn=some_address) and executed
    ...
    def likelihood_ratio_sym(self, obs, action, dist_info_old, policy_params):
        ...
        Args:
            obs (Tensor): symbolic variable for observations
            action (Tensor): symbolic variable for actions
            dist_info_old (dict): dictionary with old policy information
            policy_params (dict): dictionary of the policy parameters (each value is a Tensor)
    ...
    def get_params(self):
        """
        Get the Variables representing the trainable weights of the network (symbolic)
    ...
    def get_param_values(self):
        ...
        the same as get_params(), maybe it is the defference between tensorflow and pytorch, just keep this function.

        Returns:
            (list) : list of values for parameters
        """
        param_values = self.policy_params
        return param_values

    def get_param_for_optim(self):
        """
        Gets a list of all the current trainable parameters for optimizer

        Returns:
            (list) : list of parameters
        """
        optim_param_list = [dict(params = [self.policy_params[key]]) for key in self.policy_params.keys()]
        return optim_param_list

    def set_params(self, policy_params):
        """
        Sets the parameters for the graph
        not so sure about this function can really work, need for more test.
        Args:
        ...
        for key in params.keys():
            params[key].data = policy_params[key].data.clone()
