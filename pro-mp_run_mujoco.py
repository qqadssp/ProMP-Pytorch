...
#from meta_policy_search.envs.mujoco_envs.half_cheetah_rand_direc import HalfCheetahRandDirecEnv # import env if it is used
...
#   if I use copy.deepcopy in MetaIterativeEnvExecutor.__init__() in samplers/vectorized_env_executor.py,
#   each time I run the env.reset() for the first time it will yield a different observaions, this is not good for debug.
    env = globals()[config['env']]()
    env = normalize(env)

#    high =env.action_space.high
#    low =env.action_space.low  # high = 10. and low = -10. because normalization_scale = 10. in NormalizationEnv in envs/normalized_env.py
...
if __name__=="__main__":
    ...
    parser.add_argument('--dump_path', type=str, default='tmp')
    ...
    if args.config_file: # load configuration from json file
        ...
        config = {
            ...
            'meta_batch_size': 3, # number of sampled meta-tasks per iterations
            'num_inner_grad_steps': 2, # number of inner / adaptation gradient steps
